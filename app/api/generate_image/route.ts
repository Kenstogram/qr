import { replicateClient } from '@/utils/ReplicateImage';
 ...
 *
 * @param {LogoGenerateRequest} request - The request object to be validated.
 * @throws {Error} Error message if URL or prompt is missing.
 */

const validateRequest = (request: LogoGenerateRequest) => {
  if (!request.url) {
...
export async function POST(request: NextRequest) {
  const reqBody = (await request.json()) as LogoGenerateRequest;
  ...
  const response: LogoGenerateResponse = {
    image_url: url,
