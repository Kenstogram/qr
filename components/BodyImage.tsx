...
import { LogoGenerateRequest, LogoGenerateResponse } from '@/utils/service';
...
const BodyImage = ({
  ...
  const [response, setResponse] = useState<LogoGenerateResponse | null>(null);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 500) {
            ...
            throw new Error(`Failed to generate Image Experience: ${response.status}, ${text}`);
          }
          return;
        }
  ...
  const handleSaveExperience = async () => {
    ...
    try {
      ...
      formData.append('QrUrl', data.url);
            ...
            <form onSubmit={form.handleSubmit(handleSubmit)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="robots" {...field} />
                    </FormControl>
              ...
              />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Logo Prompt</FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder="Security guard robots with shields on the corners of a device like a map with layers in vibrant colors"
                          className="resize-none"
                          {...field}
                        />
                      </FormControl>
                      <FormDescription>This is a Sample Logo Prompt.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
