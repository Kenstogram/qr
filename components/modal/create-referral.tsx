...
export default function CreateReferralModal() {
  ...
  const [referralPercentage, setReferralPercentage] = useState(0); // New state for referral percentage
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full rounded-md bg-white dark:bg-white md:max-w-md md:border md:border-stone-200 md:shadow dark:md:border-stone-700">
      <div className="relative flex flex-col space-y-4 p-5 md:p-10">
        <h2 className="font-cal text-2xl dark:text-black">Add Brand Ambassador</h2>
        <div className="flex flex-col space-y-2">
          <label htmlFor="email" className="text-sm font-medium text-stone-500 dark:text-stone-400">Brand Ambassador Email</label>
          <input
            name="email"
            type="email"
        ...
        <div className="flex flex-col space-y-2">
          <label htmlFor="referralPercentage" className="text-sm font-medium text-stone-500 dark:text-stone-400">Referral Percentage (%)</label>
          <input
            name="commission"
            type="number"
            placeholder="20"
      ...
      <div className="flex items-center justify-end rounded-b-lg border-t border-stone-200 bg-stone-50 p-3 dark:border-stone-700 dark:bg-stone-300 md:px-10">
        <CreateReferralFormButton loading={loading} />
      </div>
    </form>
  );
}

interface CreateReferralFormButtonProps {
  loading: boolean;
}

function CreateReferralFormButton({ loading }: CreateReferralFormButtonProps) {
  return (
    <button
      type="submit"
      disabled={loading}
    ...
    >
      ...
      <p>{loading ? 'Creating Brand Ambassador...' : 'Create Brand Ambassador'}</p>
    </button>
  );
}
